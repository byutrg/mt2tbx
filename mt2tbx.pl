...
# globals, yay
my $enc = 'utf-8-strict'; # now always eq 'utf-8-strict' but you may change that
...
sub process_file {
	...
	# load mapping
	open(my $mapping,"<", $mappingFile); # true IO::Handle
	if (defined $mapping) {
		...
		print $! || em('unknown problem uploading mapping');
		return;
	}

	# load input
	open(my $in,'<',$xmlInput) || die $! || em('unknown problem uploading input');;
	...
	open(my $out,'>',File::Spec->catpath($dir, $tbxFile));
	#use File::Temp;
...
PROLOGUE
	...
	print STDOUT File::Spec->catpath($dir, $tbxFile);
}
